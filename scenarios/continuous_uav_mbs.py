...
def see_circle(x, y, x_offsets, y_offsets, map0, map1, map2):
    t_list = []
    t_list.append(map0[x + 13][y])
    t_list.append(map1[x + 13][y])
    t_list.append(map2[x + 13][y])
    t_list.append(map0[x][y + 13])
    t_list.append(map1[x][y + 13])
    t_list.append(map2[x][y + 13])
    map0 = map0[x + x_offsets, y + y_offsets]
    map1 = map1[x + x_offsets, y + y_offsets]
    map2 = map2[x + x_offsets, y + y_offsets]
    return np.concatenate([map0, map1, map2]), t_list
...
def see_local(x, y, agent_map, poi_map, obstacle_map, square=SQUARE_OBS, local_indices=None):
    if square:
        ...
        return np.concatenate([reduced_agent_obs_map, reduced_poi_obs_map, reduced_obstacle_obs_map]), t_list
    else:
        # local_obs = see_circle(x, y, local_indices[0], local_indices[1], agent_map, poi_map, obstacle_map)
        # return local_obs
        local_obs, t_list = see_circle(x, y, local_indices[0], local_indices[1], agent_map, poi_map, obstacle_map)
        return local_obs, t_list
...
class ContinuousUAVMBS:
    ...
    def get_obs(self):
        if self.pixel_obs:
            ...
            for i, (x, y) in enumerate(zip(self.x_agents, self.y_agents)):
                ...
                if self.square_obs:
                    ...
                    h_local, t_list = see_local(x, y, self.agent_map, self.poi_map, self.obstacle_map,
                                        local_indices=[self.local_indices_x, self.local_indices_y])
                    total_t_list.append(t_list)
        ...
        return obs.reshape([self.n_agent, self.obs_dim]),  total_t_list
    ...
    def step(self, actions):
        ...
        return rew, obs, adj, done, total_t_list
