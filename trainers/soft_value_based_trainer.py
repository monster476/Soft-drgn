...
class SoftValueBasedTrainer(ValueBasedTrainer):
    ...
    def _training_step(self, log_vars):
        ...
        for _ in range(hparams['training_times']):
            ...
            # batched_sample1 = self.trans_replay_buffer.sample(hparams['batch_size'])
            ...
            # f=open("filename1",'wb')
            # dict_tensors_cpu = {k: v.to('cpu') for k, v in batched_sample.items()}
            # pickle.dump(dict_tensors_cpu,f)
            # f.close()
            # print(batched_sample1)
            # f=open("filename2",'wb')
            # dict_tensors_cpu = {k: v.to('cpu') for k, v in batched_sample1.items()}
            # pickle.dump(dict_tensors_cpu,f)
            # f.close()
            # print("已保存！")
            ...
            # losses1 = {}
            # self.agent.cal_q_loss(batched_sample1, losses1, log_vars=log_vars, global_steps=self.i_iter_critic)
            # total_loss1 = sum(losses1.values())
